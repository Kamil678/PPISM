  ...
  <div class="form-container">
    <div :class="isSignIn ? 'sign-up-form active-sign-in' : 'sign-up-form'">
      <form>
        ...
        <select-component v-model="role" :options-list="optionsRole" is-filter>Rola:</select-component>
...
const submitLoginForm = async () => {
  if (validateLoginForm()) {
    ...
    $q.notify({
      position: "top-right",
      message: "Uzupełnij poprawnie pola logowania",
      color: "red",
    });
  }
};
...
const submitRegisterForm = async () => {
  if (validateRegisterForm()) {
    try {
      ...
      $q.notify({
        position: "top-right",
        message: "Utworzono konto, możesz się już zalogować",
        color: "green",
      });
    } catch (err) {
